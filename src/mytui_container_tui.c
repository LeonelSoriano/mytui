...
void mytui_inicialize_container(MytuiContainer **stance,
                                MytuiContainerParam param)
{
    if (*stance != NULL) {
...
void update_MytuiContainer(MytuiContainer *container)
{
    ...
    if (container->closeColor < 0) {
        container_close_color =
            resolve_value(_confMap, "container.close", NULL);
    } else {
        char buffer_str[255];
        sprintf(buffer_str, "%d", container->bg);
        container_close_color =
    ...
    if (container->minimizeColor < 0) {
        container_minimize_color =
            resolve_value(_confMap, "container.minimize", NULL);
    } else {
        char buffer_str[255];
    ...
    if (container->border < 0) {
        ...
        container_border =
            resolve_value(_confMap, "container.border", buffer_str);
    }
            ...
            if (container->decoration == true) {
                if (i == container->y ||
                    ...
                    nodeTranformation_add(&node, j, i, resolution_extra_color,
                           ...
                           j == (container->w + container->x) - 1) {
                    //'|'
                    nodeTranformation_add(&node, j, i, container_bg_int,
                                          container_border_int, 0x2502);
                } else {
                    nodeTranformation_add(&node, j, i, container_bg_int,
                                          MYTUI_COLOR_DEFAULT, ' ');
                }
    ...
    if (*stance != NULL) {
        ...
        while ((*stance)->childContainer != NULL) {
            ...
            if (tmpChildContainerWidget != NULL) {
                ...
                while ((*stance)->childContainer->childContainerWidgetFront !=
                       NULL) {
                    ...
                    if (tmpChildContainerWidget->next != NULL) {
                        ...
                        free((*stance)
                                 ->childContainer->childContainerWidgetFront
                                 ->widget);
...
void add_childContainerWidget(MytuiContainer **mytuiContainer, float margin_top,
                              float margin_left, float margin_bottom,
                              float margin_right, MiTuiWidget **miTuiWidget)
{
    ...
    if ((*mytuiContainer)->childContainer->childContainerWidget == NULL) {
        ...
        (*mytuiContainer)->childContainer->childContainerWidget->next =
            tmpContainerWidget;
    ...
    while (childContainer != NULL) {
        ...
        while (tmpChildContainerWidget != NULL) {
            ...
            if (tmpChildContainerWidget->widget != NULL) {
                ...
                // CONTAINER_ROT
                if (container->type == MYTUI_CONTAINER_ROOT) {
                    tmpChildContainerWidget->widget->x =
